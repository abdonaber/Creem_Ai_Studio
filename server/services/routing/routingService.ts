...
export class RoutingService {
  ...
  public static async calculateRoute(
    ...
    try {
      ...
      logger.warn(
        `[RoutingService] Primary provider ${primary.name} failed: ${primaryErr.message}. Falling back to secondary provider.`
      );

      // Try alternate provider if available before resorting to fallback
