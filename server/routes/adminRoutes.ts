...
adminRouter.get('/stats', (req: Request, res: Response) => {
  ...
  const totalRevenue = completedRides.reduce((acc, curr) => acc + (curr.finalFare || curr.estimatedFare || 0), 0);
...
// All Rides
adminRouter.get('/rides', (req: Request, res: Response) => {
  const rides = db.getAllRides();
  res.json({ success: true, data: rides });
});

// All Drivers
adminRouter.get('/drivers', (req: Request, res: Response) => {
  const drivers = db.getAllDrivers().map((d) => {
    const user = db.findUserById(d.userId);
    const vehicle = db.findVehicleByDriverId(d.id);
    return {
      ...d,
      userName: user?.name,
...
adminRouter.put('/drivers/:id/approve', (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const updated = db.updateDriver(req.params.id, { approvalStatus: status });
    if (!updated) throw new AppError('Driver not found', 404, 'NOT_FOUND');

    db.logAudit(req.user!.userId, 'DRIVER_APPROVAL_UPDATED', { driverId: req.params.id, status });

    if (io) {
      io.to(`user:${updated.userId}`).emit('notification:new', {
        title: status === 'APPROVED' ? 'تمت الموافقة على حسابك! 🎉' : 'تحديث حالة الحساب',
        body: status === 'APPROVED' ? 'تهانينا! يمكنك الآن استقبال طلبات الرحلات والبدء في تحقيق الأرباح.' : `حالة حسابك الآن: ${status}`,
        type: 'SYSTEM',
      });
    }
...
adminRouter.get('/users', (req: Request, res: Response) => {
  const users = Array.from(db.users.values()).map(({ passwordHash: _, ...u }) => u);
  res.json({ success: true, data: users });
});

// User Management: Change Status (Suspend / Activate)
adminRouter.put('/users/:id/status', (req: Request, res: Response, next: NextFunction) => {
  try {
    const { status } = req.body;
    ...
    db.logAudit(req.user!.userId, 'USER_STATUS_UPDATED', { targetUserId: req.params.id, status });
