...
describe('Ride State Machine & Concurrency Tests', () => {
  ...
  it('enforces atomic transition along the valid ride lifecycle', () => {
    const ride: IRide = {
      id: 'ride_test_1',
      riderId: 'rider_1',
      status: 'REQUESTED',
  ...
  it('rejects illegal transitions (e.g. COMPLETED -> REQUESTED or CANCELLED -> RIDE_STARTED)', () => {
    ...
    const result = db.atomicTransitionRide('ride_test_illegal', 'REQUESTED', ['SEARCHING_DRIVER']);
    expect(result.success).toBe(false);
    expect(result.error).toContain('Illegal state transition');
  });

  it('concurrency test: ensures only ONE driver can accept a ride when multiple attempt simultaneously', () => {
    const ride: IRide = {
    ...
    const attemptA = db.atomicAcceptRide('ride_race_condition', driverA);
    const attemptB = db.atomicAcceptRide('ride_race_condition', driverB);
    ...
    const updatedRide = db.findRideById('ride_race_condition');
